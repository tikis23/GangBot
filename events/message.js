...
module.exports = (client, message) => {
  ...
  //Parameters
  if (command.dev && !devs.includes(message.author.id)) return message.channel.send(":x: | Only Bot Developers can use this command!");
  if (command.guildOnly && message.channel.type == "dm") return message.channel.send(":x: | This command can only be executed in a server!");
  if (command.dmOnly && message.channel.type != "dm") return message.channel.send(":x: | This command can only be executed in DM's!");
  ...
  try {
    message.error = function(error, embed, usage, reply) {
      ...
      if (embed) return this.channel.send({embed: {title: ":x: | Error", description: `${error}${usage ? `\n\nUsage:\n\`${this.prefix}${command.name} ${usage}\`` : ""}`, color: "RED"}});
      else return this.channel.send(`:x: | ${error}${usage ? `\n\nUsage:\n\`${this.prefix}${command.name} ${usage}\`` : ""}`)
    }
    message.success = function(success, embed, reply) {
      if (!success) throw new ReferenceError("Success context is not defined");
      if (embed) return this.channel.send({embed: {title: ":white_check_mark: | Success", description: success, color: "GREEN"}})
      else return this.channel.send(`:white_check_mark: | ${success}`)
